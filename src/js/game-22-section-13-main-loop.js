...
function crRestoreHarnessState(snap){
  ...
  clearInputState();
  crResetRenderPoseHistory('harness-restore');
}
...
function runDeclarativeControlsSelfCheckBody(){
  ...
  try {
    ...
    checks.buildIdBuildScale1 = crRuntimeBuildIdentityPass();
        if(!checks.buildIdBuildScale1) errors.push('runtime BUILD_ID parity failed');
...
function runFpvGroundPlaneAlignmentSelfCheckBody(){
  ...
  try{
    checks.buildId = crRuntimeBuildIdentityPass();
...
function runContinuousFacadeTextureSelfCheckBody(){
  ...
  try{
    ...
    const materialWallSrc = typeof crDrawBuildingMaterialWallColumn === 'function' ? String(crDrawBuildingMaterialWallColumn) : '';
    checks.raycasterSamplesFacadeTexturesByContinuousFaceU =
      (composedSrc.indexOf('crDrawBuildingMaterialWallColumn') >= 0 && materialWallSrc.indexOf('crBuildingMaterialTileUForCell') >= 0) ||
      (composedSrc.indexOf('faceU') >= 0 && composedSrc.indexOf('crDrawContinuousFacadeTextureColumn') >= 0 && (composedSrc.indexOf('crGetFacadeTextureForFace') >= 0 || composedSrc.indexOf('crGetBuildingMaterialTextureForFace') >= 0));
    const simpleSrc = typeof crDrawSimpleWallColumn === 'function' ? String(crDrawSimpleWallColumn) : '';
    checks.simpleWallsBaselineFlag = crRuntimeBuildIdentityPass() && (typeof CR_SIMPLE_WALLS_BASELINE !== 'undefined' && CR_SIMPLE_WALLS_BASELINE === 1);
    checks.simpleWallRouteFirst = crRuntimeBuildIdentityPass() && (composedSrc.indexOf('CR_SIMPLE_WALLS_BASELINE') >= 0 && composedSrc.indexOf('crDrawSimpleWallColumn') >= 0 && composedSrc.indexOf('crDrawSimpleWallColumn') < composedSrc.indexOf('crDrawCalmPropsFirstWallColumn'));
    checks.simpleWallActivePathHasNoBands = crRuntimeBuildIdentityPass() && (simpleSrc.indexOf('crCalmWallBand') < 0 && simpleSrc.indexOf('crFacadeArtColBand') < 0 && simpleSrc.indexOf('crDrawContinuousFacadeTextureColumn') < 0 && simpleSrc.indexOf('crDrawSmoothBuildingFaceColumn') < 0 && simpleSrc.indexOf('fillRect(col, drawStart, 1, sliceH)') >= 0);
...
function runSpriteGroundAnchorSelfCheckBody(){
  ...
  try{
    checks.buildId = crRuntimeBuildIdentityPass();
    if(!checks.buildId) errors.push('runtime BUILD_ID parity failed');
...
function runBuildingModuleFacadeSelfCheckBody(){
  ...
  crPrepareSelfCheckPortrait();
  try {
    checks.buildIdModules1 = crRuntimeBuildIdentityPass();
    ...
    if(!checks.buildIdModules1) errors.push('runtime BUILD_ID parity failed');
...
function runFpvFacadeTargetPolishSelfCheckBody(){
  ...
  crPrepareSelfCheckPortrait();
  try {
    checks.buildIdFacadeFix1 = crRuntimeBuildIdentityPass();
    ...
    if(!checks.buildIdFacadeFix1) errors.push('runtime BUILD_ID parity failed');
...
function runFpvWallLineArtifactFixSelfCheckBody(){
  ...
  crPrepareSelfCheckPortrait();
  try {
    checks.buildIdWallLineStack = crRuntimeBuildIdentityPass();
    checks.fpvMassPreserved = CR_BUILDING_FPV_MASS >= 1.45 && CR_BUILDING_FPV_MASS <= 1.55;
    ...
    if(!checks.buildIdWallLineStack) errors.push('runtime BUILD_ID parity failed');
...
function runFpvStreetShimmerFixSelfCheckBody(){
  ...
  crPrepareSelfCheckPortrait();
  try {
    checks.buildIdShimmerFix1 = crRuntimeBuildIdentityPass();
    checks.fpvMassPreserved = CR_BUILDING_FPV_MASS >= 1.45 && CR_BUILDING_FPV_MASS <= 1.55;
    ...
    if(!checks.buildIdShimmerFix1) errors.push('runtime BUILD_ID parity failed');
    if(!checks.fpvMassPreserved) errors.push('CR_BUILDING_FPV_MASS must remain ~1.5');
...
function runStreetReadabilityMinimapSelfCheckBody(){
  ...
  crPrepareSelfCheckPortrait();
  try {
    checks.buildIdStreetRead1 = crRuntimeBuildIdentityPass();
    if(!checks.buildIdStreetRead1) errors.push('runtime BUILD_ID parity failed');
...
function runSoundFeedbackSelfCheckBody(){
  ...
  try {
    ...
    checks.buildIdOptionsClean1 = crRuntimeBuildIdentityPass();
    ...
    if(!checks.buildIdOptionsClean1) errors.push('runtime BUILD_ID parity failed');
...
function runWorldAdapterWiringPhase2SelfCheck(){
  return crWithTemporaryState('worldAdapterPhase2', () => {
    const errors = [];
    const checks = {};
    const evidence = {};
    const err0 = window.__crRuntimeErrors.length;
    try {
      ...
      // Render a frame so the raycaster wall loop runs through World.*
      if(typeof drawScene === 'function') drawScene(performance.now());
      crRenderFailureDrawFrame(performance.now());

      // 1. Zbuffer populated correctly (wall loop ran through World)
      let zbufferValid = 0;
      for(let col = 0; col < RW; col++){
        if(zbuffer[col] > 0 && zbuffer[col] < 1e8) zbufferValid++;
      }
      checks.zbufferPopulated = zbufferValid > RW * 0.3;
      evidence.zbufferValidCols = zbufferValid;

      // 2. Render failure / occlusion still works
      checks.renderFailurePasses = runRenderFailureSelfCheck().pass === true;

      // 3. Sprite occlusion screenshot proof still passes
      checks.spriteOcclusionPasses = crSpriteOcclusionScreenshotProof().pass === true;

      // 4. Raycaster invariant unbroken
      checks.raycasterInvariantPasses = runRaycasterInvariantSelfCheck().pass === true;

      // 5. Visual readability still passes (visual regression guard)
      checks.visualReadabilityPasses = runVisualReadabilitySelfCheck().pass === true;

      checks.runtimeClean = window.__crRuntimeErrors.length === err0;
      if(!checks.runtimeClean) errors.push('runtime errors during world adapter phase 2');
    } catch(e){
      errors.push(String(e && e.message ? e.message : e));
    }
...
function runFixedStepSimulationSelfCheck(){
  return crWithTemporaryState('fixedStepSimulation', () => {
    const errors = [];
    const evidence = {};
    const MOVE_TOLERANCE = 0.08;
    const err0 = window.__crRuntimeErrors.length;

    function prepWalkMap(){
      crHarnessInstallMicroMap([
        '.....',
        '.....',
        '.....',
    ...
    function simMoveFrames(n, frameDt){
      prepWalkMap();
      crResetFixedStepSimulation();
      keys['KeyW'] = true;
      const x0 = player.x, y0 = player.y;
      for(let i = 0; i < n; i++) crStepSimulationFixed(frameDt);
      delete keys['KeyW'];
      return Math.hypot(player.x - x0, player.y - y0);
    }

    const st0 = crGetFixedStepState();
    const checks = {
      fixedStepEnabled: st0.enabled === true,
      stepDtIsOneSixtieth: Math.abs(st0.stepDt - 1 / 60) < 1e-9,
      frameDtClamped: false,
      maxStepsBounded: false,
      accumulatorDoesNotExplode: true,
      movement60hzConsistent: false,
      movement30hzConsistent: false,
      backgroundSpikeDoesNotTeleport: false,
      collisionStillStopsAtWall: false,
      sprintStaminaConsistent: false,
      giveDoesNotRepeatAcrossSubsteps: false,
      lookDeltaNotMultiplied: false,
      semanticActionsStillPresent: typeof getSemanticActionMap === 'function',
      viewportAuthorityStillPasses: false,
      raycasterInvariantStillPasses: false,
      worldAdapterStillPasses: false,
      runtimeClean: true,
    };
    ...
    try {
      ...
      const d60 = simMoveFrames(60, 1 / 60);
      const d30 = simMoveFrames(30, 1 / 30);
      const d15 = simMoveFrames(15, 1 / 15);
      evidence.moveDist60 = d60;
      evidence.moveDist30 = d30;
      evidence.moveDist15 = d15;
      checks.movement60hzConsistent = Math.abs(d60 - d30) <= MOVE_TOLERANCE;
      checks.movement30hzConsistent = Math.abs(d60 - d15) <= MOVE_TOLERANCE;

      prepWalkMap();
      crResetFixedStepSimulation();
      const pxSpike = player.x, pySpike = player.y;
      const time0 = game.timeLeft;
      const spike = crStepSimulationFixed(1.25);
      evidence.spike = spike;
      checks.frameDtClamped = spike.dt <= CR_FIXED_STEP_MAX_FRAME_DT + 1e-9;
      checks.maxStepsBounded = spike.steps <= CR_FIXED_STEP_MAX_STEPS_PER_FRAME;
      checks.backgroundSpikeDoesNotTeleport = Math.hypot(player.x - pxSpike, player.y - pySpike) < 1.2;
      checks.accumulatorDoesNotExplode = crFixedStepAccumulator < CR_FIXED_STEP_DT + 1e-6;
      evidence.timeLeftDrop = time0 - game.timeLeft;
      if(evidence.timeLeftDrop > spike.steps * CR_FIXED_STEP_DT + 0.06){
        errors.push('timeLeft dropped too much on spike frame');
      }
      ...
      crResetFixedStepSimulation();
      keys['KeyW'] = true;
      for(let i = 0; i < 120; i++) crStepSimulationFixed(1 / 60);
      delete keys['KeyW'];
      checks.collisionStillStopsAtWall = !World.cellSolidAt(player.x, player.y);
      ...
      keys['KeyW'] = true;
      const stam0 = player.stamina;
      crStepSimulationFixed(1 / 30);
      evidence.sprintDropOneFrame = stam0 - player.stamina;
      delete keys['ShiftLeft'];
      delete keys['KeyW'];
      checks.sprintStaminaConsistent = evidence.sprintDropOneFrame > 0.01;

      let giveCalls = 0;
      const giveOrig = giveCan;
      giveCan = function(){
        giveCalls++;
        return giveOrig();
      };
      try {
        ...
        inp._lastGive = false;
        crResetFixedStepSimulation();
        for(let i = 0; i < 8; i++) update(CR_FIXED_STEP_DT);
        checks.giveDoesNotRepeatAcrossSubsteps = giveCalls <= 1;
      } finally {
        giveCan = giveOrig;
        inp.give = false;
        inp._lastGive = false;
      ...
      prepWalkMap();
      crResetFixedStepSimulation();
      const ang0 = player.angle;
      inp.lookDeltaRad = 0.25;
      crApplyPendingInputActions();
      const ang1 = player.angle;
      crStepSimulationFixed(0.5);
      const ang2 = player.angle;
      evidence.lookApplied = ang1 - ang0;
      evidence.lookAfterSteps = ang2 - ang1;
      checks.lookDeltaNotMultiplied =
        Math.abs(evidence.lookApplied - 0.25) < 0.03 &&
        Math.abs(evidence.lookAfterSteps) < 0.02;

      checks.viewportAuthorityStillPasses = runViewportAuthoritySelfCheck().pass;
      checks.worldAdapterStillPasses = runWorldLayerAdapterSelfCheck().pass;
      checks.raycasterInvariantStillPasses = runRaycasterInvariantSelfCheck().pass;
      checks.runtimeClean = window.__crRuntimeErrors.length === err0;
      if(!checks.runtimeClean) errors.push('runtime errors during fixed-step selfcheck');
    } catch(e){
      errors.push(String(e && e.message ? e.message : e));
    }
...
function runSingleMaterialBuildingTextureSelfCheck(){
  if(_crHarnessDepth > 0) return runSingleMaterialBuildingTextureSelfCheckBody();
  return crWithTemporaryState('singleMaterialBuildingTextures', () => runSingleMaterialBuildingTextureSelfCheckBody());
}

function runSingleMaterialBuildingTextureSelfCheckBody(){
  const errors = [];
  const checks = {};
  const evidence = { scenarios: [], materialsSeen: {} };
  const err0 = window.__crRuntimeErrors.length;
  try {
    const atlas = crBuildFacadeTextureAtlas();
    checks.atlasHasStucco = !!atlas.material_stucco;
    checks.atlasHasRedBrick = !!atlas.material_red_brick;
    ...
    checks.liveDrawSceneUsesBuildingMaterialPath =
      ...
      drawSrc.indexOf('crDrawBuildingMaterialWallColumn') >= 0 &&
      drawSrc.indexOf('!useBuildingMaterialTextures') >= 0;
    const composedSrc = typeof crDrawComposedFacadeFaceColumn === 'function' ? String(crDrawComposedFacadeFaceColumn) : '';
    const calmIdx = composedSrc.indexOf('crDrawCalmPropsFirstWallColumn');
    const singleIdx = composedSrc.indexOf('CR_SINGLE_MATERIAL_BUILDING_TEXTURES');
    checks.composedFacadePrioritizesSingleMaterial =
      singleIdx >= 0 &&
      composedSrc.indexOf('crDrawBuildingMaterialWallColumn') >= 0 &&
      (calmIdx < 0 || singleIdx < calmIdx);

    const scenarios = [
      { district: 1, seed: 880101 },
      { district: 2, seed: 880102 },
      { district: 3, seed: 903203 },
      { district: 4, seed: 934204 },
      { district: 2, seed: 914201 },
      { district: 3, seed: 914203 },
    ];

    for(const sc of scenarios){
      crSetSelectedStartDistrict(sc.district);
      genCity(sc.seed, sc.district, '');
      const row = { seed: sc.seed, district: sc.district, buildings: 0, cells: 0, materials: {}, failures: [] };
      const registry = game.buildingRegistry || {};
      for(const bid of Object.keys(registry)){
        const reg = registry[bid];
        if(!reg) continue;
        const mat = crNormalizeBuildingTextureMaterial(reg.materialKey);
        row.buildings++;
        row.materials[mat] = (row.materials[mat] || 0) + 1;
        evidence.materialsSeen[mat] = (evidence.materialsSeen[mat] || 0) + 1;
      }
      // Walk every building cell, verify face material equals building material.
      for(let y = 0; y < game.MAP_H; y++){
        for(let x = 0; x < game.MAP_W; x++){
          const cell = game.buildingGrid && game.buildingGrid[y] && game.buildingGrid[y][x];
          if(!cell) continue;
          row.cells++;
          const reg = game.buildingRegistry && game.buildingRegistry[cell.bid];
          if(!reg) { row.failures.push({ x, y, why: 'no-reg' }); continue; }
          const expectedMat = crNormalizeBuildingTextureMaterial(reg.materialKey);
          for(const face of ['north','south','east','west']){
            const resolved = crGetBuildingMaterialTextureForFace(x, y, face);
            if(!resolved || resolved.materialKey !== expectedMat){
              row.failures.push({ x, y, face, expected: expectedMat, got: resolved && resolved.materialKey });
            }
          }
        }
      }
      evidence.scenarios.push(row);
      if(row.failures.length) errors.push('single-material failures in seed ' + sc.seed + ' district ' + sc.district + ' (' + row.failures.length + ')');
    }

    checks.allFourMaterialsSeen = CR_BUILDING_TEXTURE_MATERIALS.every(k => (evidence.materialsSeen[k] || 0) > 0);
    checks.noFaceMaterialMismatch = evidence.scenarios.every(s => s.failures.length === 0);
    checks.raycasterInvariantStillPasses = runRaycasterInvariantSelfCheck().pass === true;
    checks.worldAdapterPhase2StillPasses = runWorldAdapterWiringPhase2SelfCheck().pass === true;
    checks.worldAdapterPhase1StillPasses = runWorldAdapterWiringPhase1SelfCheck().pass === true;
    checks.spriteOcclusionStillPasses = crSpriteOcclusionScreenshotProof().pass === true;
    checks.fixedStepStillPasses = runFixedStepSimulationSelfCheck().pass === true;
    checks.runtimeClean = window.__crRuntimeErrors.length === err0;
    if(!checks.allFourMaterialsSeen) errors.push('not all four materials seen across scenarios');
    if(!checks.raycasterInvariantStillPasses) errors.push('raycaster invariant regressed');
    if(!checks.worldAdapterPhase1StillPasses) errors.push('world adapter phase 1 regressed');
    if(!checks.worldAdapterPhase2StillPasses) errors.push('world adapter phase 2 regressed');
    if(!checks.spriteOcclusionStillPasses) errors.push('sprite occlusion regressed');
    if(!checks.fixedStepStillPasses) errors.push('fixed-step regressed');
    if(!checks.runtimeClean) errors.push('runtime errors during single-material selfcheck');
  } catch(e) {
    errors.push(String(e && e.message ? e.message : e));
  }
  for(const k of Object.keys(checks)) if(!checks[k]) errors.push('check failed: ' + k);
  return {
...
function crTextureStats(canvas){
  if(!canvas || !canvas.getContext) return { min: 0, max: 0, mean: 0, range: 0 };
  ...
  return { min, max, mean: n ? sum / n : 0, range: max - min };
}

function crInstallFacadeSkinProofScene(mode){
  const mat = mode === 'service' ? 'light_gray_cinderblock' : mode === 'house' ? 'stucco' : 'red_brick';
  const family = mode === 'house' ? 'house' : mode === 'service' ? 'service' : 'storefront';
  const moduleId = family === 'storefront' ? 'storefront_small_a' : family === 'service' ? 'garage_service_a' : 'house_small_a';
  game.facadeSkinProofKind = family;
  const wCells = mode === 'longface' ? 6 : 4;
  const GW = 11, GH = 9;
  const map = [];
  for(let y = 0; y < GH; y++){
    const row = [];
  ...
  const x0 = 3, y0 = 2, x1 = x0 + wCells - 1;
  for(let x = x0; x <= x1; x++) map[y0][x] = WALL.BUILDING;
  game.map = map;
  game.MAP_W = GW;
  game.MAP_H = GH;
  game.buildingRegistry = { bench_wall: { moduleId, x0, y0, front: 'south', mod: { w: wCells, h: 1 }, materialKey: mat } };
  game.buildingGrid = [];
  for(let y = 0; y < GH; y++) game.buildingGrid.push(new Array(GW).fill(null));
  for(let x = x0; x <= x1; x++) game.buildingGrid[y0][x] = { bid: 'bench_wall', lx: x - x0, ly: 0, mid: 'bench' };
  game.wallShade = [];
  for(let y = 0; y < GH; y++){
    const row = [];
  ...
  game.facadeSkinProofProfile = null;
  return { mode, materialKey: mat, wCells, player: { x: player.x, y: player.y, angle: player.angle } };
}

function crInstallMaterialTextureBenchScene(materialKey){
  const mat = crNormalizeBuildingTextureMaterial(materialKey);
  const GW = 11, GH = 9;
  const map = [];
  for(let y = 0; y < GH; y++){
    const row = [];
  ...
  for(let x = 3; x <= 7; x++) map[2][x] = WALL.BUILDING;
  game.map = map;
  game.MAP_W = GW;
  game.MAP_H = GH;
  game.buildingRegistry = { bench_wall: { moduleId: 'bench', x0: 3, y0: 2, front: 'south', mod: {}, materialKey: mat } };
  game.buildingGrid = [];
  for(let y = 0; y < GH; y++) game.buildingGrid.push(new Array(GW).fill(null));
  for(let x = 3; x <= 7; x++) game.buildingGrid[2][x] = { bid: 'bench_wall', lx: x - 3, ly: 0, mid: 'bench' };
  game.wallShade = [];
  for(let y = 0; y < GH; y++){
    const row = [];
    for(let x = 0; x < GW; x++) row.push(0.5);
  ...
  if(typeof crBuildBuildingMaterialComponents === 'function') crBuildBuildingMaterialComponents(map);
  if(typeof crSyncRegistryMaterialsToComponents === 'function') crSyncRegistryMaterialsToComponents();
  return { materialKey: mat, player: { x: player.x, y: player.y, angle: player.angle } };
}

function runWalltextures2ScaleVariationSelfCheck(){
  if(_crHarnessDepth > 0) return runWalltextures2ScaleVariationSelfCheckBody();
  return crWithTemporaryState('walltextures2ScaleVariation', () => runWalltextures2ScaleVariationSelfCheckBody());
}

function runWalltextures2ScaleVariationSelfCheckBody(){
  const errors = [];
  const checks = {};
  const evidence = {};
  const err0 = window.__crRuntimeErrors.length;
  try {
    checks.oneMaterialPerBuilding = runSingleMaterialBuildingTextureSelfCheck().pass === true;

    const atlas = crBuildFacadeTextureAtlas();
    checks.materialAtlasHasFour = !!(atlas.material_stucco && atlas.material_red_brick && atlas.material_light_gray_cinderblock && atlas.material_aluminum_siding);

    const renderSrc = typeof drawScene === 'function' ? String(drawScene) : '';
    const helperSrc = typeof crDrawBuildingMaterialWallColumn === 'function' ? String(crDrawBuildingMaterialWallColumn) : '';
    checks.liveDrawSceneUsesMaterialTextures =
      renderSrc.indexOf('buildingMaterialWall') >= 0 &&
      renderSrc.indexOf('crDrawBuildingMaterialWallColumn') >= 0;
    checks.baseTextureUsesTiledWorldU =
      helperSrc.indexOf('crBuildingMaterialTileUForCell') >= 0 &&
      renderSrc.indexOf('crDrawBuildingMaterialWallColumn') >= 0;
    checks.overlayUsesFacadeU =
      helperSrc.indexOf('crDrawFpvFacadePackRoleOverlays') >= 0 &&
      helperSrc.indexOf('wallX') >= 0 &&
      helperSrc.indexOf('materialU') >= 0;

    const uA = crBuildingMaterialTileUForCell(3, 2, 'south', 0.15, 'light_gray_cinderblock');
    const uB = crBuildingMaterialTileUForCell(3, 2, 'south', 0.85, 'light_gray_cinderblock');
    const uStucco = crBuildingMaterialTileUForCell(5, 2, 'south', 0.3, 'stucco');
    const uBrick = crBuildingMaterialTileUForCell(5, 2, 'south', 0.3, 'red_brick');
    checks.tileUVariesAcrossCells = Math.abs(uA - uB) > 0.12;
    checks.tileURespectsMaterialScale = Math.abs(uStucco - uBrick) > 0.05;
    evidence.tileSamples = { uA, uB, uStucco, uBrick };

    const spec = typeof crMaterialTextureDebugSpec === 'function' ? crMaterialTextureDebugSpec() : null;
    const cinderSpec = spec && spec.light_gray_cinderblock;
    checks.cinderblockSeamsNotDominant = !!(cinderSpec && cinderSpec.aspect >= 2.5 && cinderSpec.blockW >= 40);
    const stuccoSrc = typeof crPaintMaterialStucco === 'function' ? String(crPaintMaterialStucco) : '';
    checks.stuccoHasReadableVariance = stuccoSrc.indexOf('150') >= 0 && stuccoSrc.indexOf('320') >= 0;

    const overlaySrc = typeof crDrawFpvFacadePackRoleOverlays === 'function' ? String(crDrawFpvFacadePackRoleOverlays) : '';
    checks.noRoleMaterialBaseOverride = overlaySrc.indexOf('crDrawFpvFacadePackMaterialBase') < 0;

    const stStats = crTextureStats(atlas.material_stucco);
    const cbStats = crTextureStats(atlas.material_light_gray_cinderblock);
    evidence.stuccoStats = stStats;
    evidence.cinderStats = cbStats;
    checks.stuccoTextureHasLumaRange = stStats.range >= 12;
    checks.cinderTextureHasLumaRange = cbStats.range >= 10;

    checks.noRuntimeErrors = window.__crRuntimeErrors.length === err0;
    if(!checks.tileUVariesAcrossCells) errors.push('material tile U does not vary along wall');
    if(!checks.tileURespectsMaterialScale) errors.push('material tile scale not applied per material');
    if(!checks.stuccoTextureHasLumaRange) errors.push('stucco atlas luma range too flat');
    if(!checks.cinderTextureHasLumaRange) errors.push('cinderblock atlas luma range too flat');
  } catch(e) {
    errors.push(String(e && e.message ? e.message : e));
  }
  for(const k of Object.keys(checks)) if(!checks[k]) errors.push('check failed: ' + k);
...
function runWalltextures3WholeBuildingTextureOwnershipSelfCheck(){
  if(_crHarnessDepth > 0) return runWalltextures3WholeBuildingTextureOwnershipSelfCheckBody();
  return crWithTemporaryState('walltextures3WholeBuildingTextureOwnership', () => runWalltextures3WholeBuildingTextureOwnershipSelfCheckBody());
}

function runWalltextures3WholeBuildingTextureOwnershipSelfCheckBody(){
  const errors = [];
  const checks = {};
  const evidence = { scenarios: [], coveredNonRegisteredWallCells: 0 };
  const err0 = window.__crRuntimeErrors.length;
  const matFnSrc = typeof crBuildingMaterialForCell === 'function' ? String(crBuildingMaterialForCell) : '';
  const drawHelperSrc = typeof crDrawBuildingMaterialWallColumn === 'function' ? String(crDrawBuildingMaterialWallColumn) : '';

  checks.buildIdWalltextures3 = crRuntimeBuildIdentityPass();
  checks.liveDrawUsesComponentMaterial = matFnSrc.indexOf('buildingMaterialGrid') >= 0;
  checks.walltextures2TilingPreserved =
    drawHelperSrc.indexOf('crBuildingMaterialTileUForCell') >= 0 &&
    drawHelperSrc.indexOf('crDrawFpvFacadePackRoleOverlays') >= 0;
  checks.overlaysStillPresent = drawHelperSrc.indexOf('crDrawFpvFacadePackRoleOverlays') >= 0;

  const scenarios = [
    { district: 1, seed: 880101 },
    { district: 2, seed: 290144176 },
    { district: 2, seed: 880102 },
    { district: 3, seed: 880103 },
    { district: 4, seed: 880104 },
  ];
  ...
  try {
    for(const sc of scenarios){
      ...
      const row = {
        ...
        seed: sc.seed,
        wallCells: 0,
        components: Object.keys(game.buildingMaterialComponents || {}).length,
        nonRegisteredCovered: 0,
        componentMaterialMismatches: 0,
        registryMismatches: 0,
        missingGrid: 0,
      };

      checks.componentGridExists = !!game.buildingMaterialGrid && game.buildingMaterialGrid.length === game.MAP_H;
      checks.componentsExist = row.components > 0;

      const compMats = {};
      for(let y = 0; y < game.MAP_H; y++){
        for(let x = 0; x < game.MAP_W; x++){
          const wt = map[y] && map[y][x];
          if(!crIsBuildingMaterialWallType(wt)) continue;
          row.wallCells++;
          const own = game.buildingMaterialGrid[y] && game.buildingMaterialGrid[y][x];
          if(!own || !own.materialKey){ row.missingGrid++; continue; }
          const mat = crNormalizeBuildingTextureMaterial(own.materialKey);
          const cid = own.componentId;
          if(!compMats[cid]) compMats[cid] = new Set();
          compMats[cid].add(mat);

          const bg = game.buildingGrid && game.buildingGrid[y] && game.buildingGrid[y][x];
          if(!bg) row.nonRegisteredCovered++;
          else {
            const reg = game.buildingRegistry && game.buildingRegistry[bg.bid];
            const regMat = crNormalizeBuildingTextureMaterial(reg && reg.materialKey);
            if(regMat !== mat) row.registryMismatches++;
          }
        }
      }

      for(const cid of Object.keys(compMats)){
        if(compMats[cid].size !== 1) row.componentMaterialMismatches++;
      }

      evidence.coveredNonRegisteredWallCells += row.nonRegisteredCovered;
      evidence.scenarios.push(row);
      if(row.missingGrid) errors.push('missing component material at grid cells seed ' + sc.seed);
      if(row.componentMaterialMismatches) errors.push('component has multiple materials seed ' + sc.seed);
      if(row.registryMismatches) errors.push('registry vs component mismatch seed ' + sc.seed);
    }

    checks.allBuildingWallsHaveComponentMaterial = evidence.scenarios.every(s => s.missingGrid === 0 && s.wallCells > 0);
    checks.eachComponentHasOneMaterial = evidence.scenarios.every(s => s.componentMaterialMismatches === 0);
    checks.registryCellsMatchComponentMaterial = evidence.scenarios.every(s => s.registryMismatches === 0);
    checks.nonRegisteredWallCellsCovered = evidence.coveredNonRegisteredWallCells > 0;
    checks.noRuntimeErrors = window.__crRuntimeErrors.length === err0;
  } catch(e) {
    errors.push(String(e && e.message ? e.message : e));
  }
...
function runWalltextures4MaterialShapeReadabilitySelfCheck(){
  if(_crHarnessDepth > 0) return runWalltextures4MaterialShapeReadabilitySelfCheckBody();
  return crWithTemporaryState('walltextures4MaterialShapeReadability', () => runWalltextures4MaterialShapeReadabilitySelfCheckBody());
}

function runWalltextures4MaterialShapeReadabilitySelfCheckBody(){
  const errors = [];
  const checks = {};
  const evidence = {};
  const err0 = window.__crRuntimeErrors.length;
  const matFnSrc = typeof crBuildingMaterialForCell === 'function' ? String(crBuildingMaterialForCell) : '';
  const sidingSrc = typeof crPaintMaterialAluminumSiding === 'function' ? String(crPaintMaterialAluminumSiding) : '';

  try {
    checks.buildId = crRuntimeBuildIdentityPass();
    checks.componentOwnershipPreserved =
      matFnSrc.indexOf('buildingMaterialGrid') >= 0 &&
      typeof crBuildBuildingMaterialComponents === 'function' &&
      typeof crSyncRegistryMaterialsToComponents === 'function';

    const spec = typeof crMaterialTextureDebugSpec === 'function' ? crMaterialTextureDebugSpec() : null;
    evidence.cinderblockSpec = spec && spec.light_gray_cinderblock;
    evidence.sidingSpec = spec && spec.aluminum_siding;
    evidence.materialScales = {
      cinderblock: typeof crMaterialTileScaleCells === 'function' ? crMaterialTileScaleCells('light_gray_cinderblock') : null,
      siding: typeof crMaterialTileScaleCells === 'function' ? crMaterialTileScaleCells('aluminum_siding') : null,
    };

    checks.atlasHasFourMaterials = !!(crBuildFacadeTextureAtlas().material_stucco &&
      crBuildFacadeTextureAtlas().material_red_brick &&
      crBuildFacadeTextureAtlas().material_light_gray_cinderblock &&
      crBuildFacadeTextureAtlas().material_aluminum_siding);

    checks.cinderblockRectangularSpec = !!(evidence.cinderblockSpec && evidence.cinderblockSpec.aspect >= 2.5 && evidence.cinderblockSpec.staggeredRows);
    checks.cinderblockTileScaleWidened = evidence.materialScales.cinderblock >= 1.8;
    checks.sidingHorizontalBandSpec = !!(evidence.sidingSpec && evidence.sidingSpec.orientation === 'horizontal' && evidence.sidingSpec.bandH >= 8);
    checks.sidingTileScaleWidened = evidence.materialScales.siding >= 1.9;
    checks.sidingLowNoisePainter = sidingSrc.indexOf('for(let y = 0; y < h; y += bandH)') >= 0 && sidingSrc.indexOf('for(let i = 0; i < 14; i++)') < 0;

    const own = runWalltextures3WholeBuildingTextureOwnershipSelfCheck();
    checks.singleMaterialOwnershipStillPasses = own.pass === true;
    evidence.ownership = { pass: own.pass, checks: own.checks };

    const wt2 = runWalltextures2ScaleVariationSelfCheck();
    checks.walltextures2ScaleStillPasses = wt2.pass === true;
    evidence.walltextures2 = { pass: wt2.pass };

    checks.raycasterInvariantPasses = runRaycasterInvariantSelfCheck().pass === true;
    checks.runtimeClean = window.__crRuntimeErrors.length === err0;

    if(!checks.buildId) errors.push('runtime BUILD_ID parity failed');
    if(!checks.cinderblockRectangularSpec) errors.push('cinderblock aspect/stagger spec failed');
    if(!checks.sidingHorizontalBandSpec) errors.push('siding horizontal band spec failed');
    if(!checks.sidingLowNoisePainter) errors.push('siding painter still has high-frequency noise loops');
  } catch(e) {
    errors.push(String(e && e.message ? e.message : e));
  }
  for(const k of Object.keys(checks)) if(!checks[k]) errors.push('check failed: ' + k);
...
function runDecalIntegration1SelfCheck(){
  if(_crHarnessDepth > 0) return runDecalIntegration1SelfCheckBody();
  return crWithTemporaryState('decalIntegration1', () => runDecalIntegration1SelfCheckBody());
}

function runDecalIntegration1SelfCheckBody(){
  const errors = [];
  const checks = {};
  const evidence = {};
  const err0 = window.__crRuntimeErrors.length;
  ...
  try {
    ...
    // walltextures3 ownership still preserved
    const own = runWalltextures3WholeBuildingTextureOwnershipSelfCheck();
    checks.walltextures3OwnershipPreserved = own.pass === true;
    evidence.walltextures3Ownership = { pass: own.pass };

    // walltextures4 material readability still passes
    const wt4 = runWalltextures4MaterialShapeReadabilitySelfCheck();
    checks.walltextures4ReadabilityPreserved = wt4.pass === true;
    evidence.walltextures4 = { pass: wt4.pass };

    // decal pack exists
    const spec = typeof crDecalDebugSpec === 'function' ? crDecalDebugSpec() : null;
    evidence.decalSpec = spec;
    checks.decalPackExists = !!(spec && spec.version === 'decalintegration1');
    checks.requiredDecalKeysPresent = !!(spec && spec.decalKeys && spec.decalKeys.length >= 11);

    // verify all required families are represented
    if(spec && spec.families){
      const fam = spec.families;
      checks.residentialDecals = !!(fam.residential && fam.residential.length >= 4);
      checks.utilityDecals = !!(fam.utility && fam.utility.length >= 3);
      checks.commercialDecals = !!(fam.commercial && fam.commercial.length >= 3);
    } else {
      checks.residentialDecals = false;
      checks.utilityDecals = false;
      checks.commercialDecals = false;
    }

    // sign text fitting helper exists
    checks.signTextFitHelperExists = typeof crFitSignText === 'function';
    if(checks.signTextFitHelperExists){
      const fitTest = crFitSignText('MAIN STREET BAGELS', 120, 24);
      checks.signTextNeverOverflows = fitTest.overflow === false && fitTest.lines.length > 0;
      evidence.signFitSample = fitTest;
    } else {
      checks.signTextNeverOverflows = false;
    }

    // decal overlay function exists
    checks.decalOverlayDrawExists = typeof crDrawDecalOverlayColumn === 'function';

    // verify overlay is called from material wall column (not replacing)
    const matWallSrc = typeof crDrawBuildingMaterialWallColumn === 'function' ? String(crDrawBuildingMaterialWallColumn) : '';
    checks.decalCalledAsOverlay = matWallSrc.indexOf('crDrawDecalOverlayColumn') >= 0;
    checks.materialBaseStillDrawn = matWallSrc.indexOf('crDrawContinuousFacadeTextureColumn') >= 0;

    // generate city and check at least one face gets decals
    const saved = { seed: game.seed, district: game.district };
    try {
      game.seed = 290144176;
      game.district = 2;
      const r = game.seed;
      genCity(game.seed, game.district, game.modifier || '');
      if(typeof crBuildBuildingMaterialComponents === 'function') crBuildBuildingMaterialComponents(game.map);
      if(typeof crSyncRegistryMaterialsToComponents === 'function') crSyncRegistryMaterialsToComponents();

      let signFound = false;
      let residentialFound = false;
      let commercialFound = false;
      let utilityFound = false;
      const decalCounts = {};
      for(let y = 1; y < game.MAP_H - 1; y++){
        for(let x = 1; x < game.MAP_W - 1; x++){
          for(const fd of ['south', 'north', 'east', 'west']){
            const placements = crGetDecalsForFace(x, y, fd);
            for(const p of placements){
              decalCounts[p.key] = (decalCounts[p.key] || 0) + 1;
              if(p.key === 'decal_sign_board'){ signFound = true; commercialFound = true; }
              if(p.key === 'decal_front_door' || p.key === 'decal_house_window' || p.key === 'decal_mailbox_attached' || p.key === 'decal_porch_light' || p.key === 'decal_house_numbers') residentialFound = true;
              if(p.key === 'decal_service_door' || p.key === 'decal_utility_window' || p.key === 'decal_electric_meter') utilityFound = true;
              if(p.key === 'decal_storefront_window' || p.key === 'decal_glass_door') commercialFound = true;
            }
          }
        }
      }
      evidence.decalCounts = decalCounts;
      checks.signDecalAssigned = signFound;
      checks.residentialDecalAssigned = residentialFound;
      checks.commercialDecalAssigned = commercialFound;
      checks.utilityDecalAssigned = utilityFound;
    } finally {
      game.seed = saved.seed;
      game.district = saved.district;
    }

    checks.runtimeClean = window.__crRuntimeErrors.length === err0;

    if(!checks.walltextures3OwnershipPreserved) errors.push('walltextures3 ownership not preserved');
    if(!checks.decalPackExists) errors.push('decal pack missing');
    if(!checks.signTextFitHelperExists) errors.push('crFitSignText missing');
    if(!checks.decalOverlayDrawExists) errors.push('crDrawDecalOverlayColumn missing');
    if(!checks.decalCalledAsOverlay) errors.push('decal overlay not called from material wall column');
    if(!checks.signDecalAssigned) errors.push('no sign decals assigned in generated city');
    if(!checks.residentialDecalAssigned) errors.push('no residential decals assigned');
    if(!checks.commercialDecalAssigned) errors.push('no commercial decals assigned');
  } catch(e) {
    errors.push(String(e && e.message ? e.message : e));
  }
  for(const k of Object.keys(checks)) if(!checks[k]) errors.push('check failed: ' + k);
...
function runDecalIntegration2SelfCheck(){
  if(_crHarnessDepth > 0) return runDecalIntegration2SelfCheckBody();
  return crWithTemporaryState('decalIntegration2', () => runDecalIntegration2SelfCheckBody());
}

function runDecalIntegration2SelfCheckBody(){
  const errors = [];
  const checks = {};
  const evidence = {};
  const err0 = window.__crRuntimeErrors.length;
  ...
  try {
    ...
    // Nested prior proofs
    const own = runWalltextures3WholeBuildingTextureOwnershipSelfCheck();
    checks.walltextures3OwnershipStillPasses = own.pass === true;
    evidence.walltextures3 = { pass: own.pass };

    const wt4 = runWalltextures4MaterialShapeReadabilitySelfCheck();
    checks.walltextures4StillPasses = wt4.pass === true;
    evidence.walltextures4 = { pass: wt4.pass };

    const di1 = runDecalIntegration1SelfCheck();
    checks.decalIntegration1StillPasses = di1.pass === true;
    evidence.decalIntegration1 = { pass: di1.pass };

    // Material stability specs
    const spec = typeof crMaterialTextureDebugSpec === 'function' ? crMaterialTextureDebugSpec() : null;
    evidence.materialSpecs = spec;
    if(spec && spec.red_brick){
      checks.redBrickStableSpec = spec.red_brick.brickW >= 32 && spec.red_brick.brickH >= 10 && spec.red_brick.tileScaleCells >= 1.4;
    } else { checks.redBrickStableSpec = false; }
    if(spec && spec.aluminum_siding){
      checks.sidingStableSpec = spec.aluminum_siding.bandH >= 11 && spec.aluminum_siding.tileScaleCells >= 2.3;
    } else { checks.sidingStableSpec = false; }

    // Material LOD helper exists
    checks.materialLodHelperExists = typeof crMaterialTextureKeyForSlice === 'function';

    // Component fallback exists
    checks.componentDecalFallbackExists = typeof crBuildingMaterialComponentForCell === 'function' && typeof crGetComponentDecalsForFace === 'function';

    // Sign fit still safe
    if(typeof crFitSignText === 'function'){
      const fit = crFitSignText('COLORADO NATIONAL MONUMENT', 100, 20);
      checks.signFitStillPasses = fit.overflow === false && fit.lines.length > 0;
      evidence.signFitSample = fit;
    } else { checks.signFitStillPasses = false; }

    // Draw path still calls base material first
    const matWallSrc = typeof crDrawBuildingMaterialWallColumn === 'function' ? String(crDrawBuildingMaterialWallColumn) : '';
    checks.decalsOverlayOnly = matWallSrc.indexOf('crDrawContinuousFacadeTextureColumn') >= 0 && matWallSrc.indexOf('crDrawDecalOverlayColumn') >= 0;

    // Decal density check: generate city and count
    const saved = { seed: game.seed, district: game.district };
    try {
      game.seed = 290144176;
      game.district = 2;
      ...
      let registryDecals = 0, componentDecals = 0;
      let signCount = 0, commercialCount = 0, utilityCount = 0, residentialCount = 0;
      const decalCounts = {};
      for(let y = 1; y < game.MAP_H - 1; y++){
        for(let x = 1; x < game.MAP_W - 1; x++){
          const cell = game.buildingGrid && game.buildingGrid[y] && game.buildingGrid[y][x];
          const reg = cell && game.buildingRegistry && game.buildingRegistry[cell.bid];
          const isRegistry = !!reg;
          for(const fd of ['south', 'north', 'east', 'west']){
            const placements = crGetDecalsForFace(x, y, fd);
            for(const p of placements){
              if(isRegistry) registryDecals++; else componentDecals++;
              decalCounts[p.key] = (decalCounts[p.key] || 0) + 1;
              if(p.key === 'decal_sign_board'){ signCount++; commercialCount++; }
              if(p.key === 'decal_storefront_window' || p.key === 'decal_glass_door') commercialCount++;
              if(p.key === 'decal_service_door' || p.key === 'decal_utility_window' || p.key === 'decal_electric_meter') utilityCount++;
              if(p.key === 'decal_front_door' || p.key === 'decal_house_window' || p.key === 'decal_mailbox_attached' || p.key === 'decal_porch_light' || p.key === 'decal_house_numbers') residentialCount++;
            }
          }
        }
      }
      evidence.decalCounts = decalCounts;
      evidence.registryDecals = registryDecals;
      evidence.componentDecals = componentDecals;
      evidence.signCount = signCount;
      checks.visibleDecalDensity = (signCount + commercialCount + utilityCount + residentialCount) >= 20;
      checks.nonRegistryWallCellsCanReceiveDecals = componentDecals > 0;
    } finally {
      game.seed = saved.seed;
      game.district = saved.district;
    }
...
function runDecalIntegration3VisualSelfCheck(){
  if(_crHarnessDepth > 0) return runDecalIntegration3VisualSelfCheckBody();
  return crWithTemporaryState('decalIntegration3', () => runDecalIntegration3VisualSelfCheckBody());
}

function runDecalIntegration3VisualSelfCheckBody(){
  const errors = [];
  const checks = {};
  const evidence = {};
  const metrics = { d2LongWall: {}, brickMotionProbe: {}, decalCoverage: {} };
  const err0 = window.__crRuntimeErrors.length;
  ...
  try {
    ...
    const di2 = runDecalIntegration2SelfCheck();
    checks.decalIntegration2CompatibilityStillPasses = di2.pass === true;
    evidence.decalIntegration2 = { pass: di2.pass };

    // Material stability specs — stricter thresholds for di3
    const spec = typeof crMaterialTextureDebugSpec === 'function' ? crMaterialTextureDebugSpec() : null;
    evidence.materialSpecs = spec;
    if(spec && spec.red_brick){
      checks.redBrickLowFrequency = spec.red_brick.brickW >= 32 && spec.red_brick.brickW <= 38 &&
        spec.red_brick.brickH >= 9 && spec.red_brick.brickH <= 11 &&
        spec.red_brick.tileScaleCells >= 1.8 && spec.red_brick.tileScaleCells <= 2.2;
      metrics.brickSpec = { brickW: spec.red_brick.brickW, brickH: spec.red_brick.brickH, tileScale: spec.red_brick.tileScaleCells, aspect: spec.red_brick.aspect };
      if(spec.light_gray_cinderblock){
        const rb = spec.red_brick;
        const cb = spec.light_gray_cinderblock;
        checks.brickNotCinderGeometry = rb.brickW !== cb.blockW && rb.brickH !== cb.blockH &&
          Math.abs(rb.aspect - cb.aspect) >= 0.6;
        checks.brickGeometryDistinctFromCinder = checks.brickNotCinderGeometry;
      } else {
        checks.brickNotCinderGeometry = false;
        checks.brickGeometryDistinctFromCinder = false;
      }
    } else { checks.redBrickLowFrequency = false; checks.brickNotCinderGeometry = false; checks.brickGeometryDistinctFromCinder = false; }
    if(spec && spec.aluminum_siding){
      checks.sidingLowFrequency = spec.aluminum_siding.bandH >= 15 && spec.aluminum_siding.tileScaleCells >= 2.5;
      metrics.sidingSpec = { bandH: spec.aluminum_siding.bandH, tileScale: spec.aluminum_siding.tileScaleCells };
    } else { checks.sidingLowFrequency = false; }

    // Material identity stable: calm atlas keys exist + LOD function wired
    const atlas = typeof crBuildFacadeTextureAtlas === 'function' ? crBuildFacadeTextureAtlas() : null;
    checks.materialIdentityStable = !!(atlas && atlas.material_red_brick_calm && atlas.material_aluminum_siding_calm);
    checks.noDistanceGreyFlip = typeof crMaterialTextureKeyForSlice === 'function' &&
      crMaterialTextureKeyForSlice('red_brick', 30) === 'material_red_brick_calm' &&
      crMaterialTextureKeyForSlice('red_brick', 200) === 'material_red_brick' &&
      crMaterialTextureKeyForSlice('aluminum_siding', 30) === 'material_aluminum_siding_calm' &&
      crMaterialTextureKeyForSlice('aluminum_siding', 200) === 'material_aluminum_siding';

    if(atlas && typeof crSampleMaterialTexturePaletteStats === 'function'){
      ...
      evidence.paletteSamples = { brick: brickPal, cinder: cinderPal };
      if(brickPal && cinderPal){
        checks.brickPaletteDistinctFromCinder = brickPal.warmBrickFraction >= 0.10 && cinderPal.grayBlockFraction >= 0.06 &&
          (brickPal.avgR - brickPal.avgB) >= 45 && (brickPal.avgR - brickPal.avgG) >= 35 &&
          Math.abs(cinderPal.avgR - cinderPal.avgG) < 28;
        checks.brickNotMostlyGrayPixels = brickPal.grayBlockFraction < 0.55;
        metrics.paletteDistance = Math.abs(brickPal.avgR - cinderPal.avgR) + Math.abs(brickPal.avgG - cinderPal.avgG);
      } else {
        checks.brickPaletteDistinctFromCinder = false;
        checks.brickNotMostlyGrayPixels = false;
      }
      checks.brickCalmHasVerticalJoints = typeof crCalmBrickHasVerticalJoints === 'function' &&
        crCalmBrickHasVerticalJoints(atlas.material_red_brick_calm);
      checks.cinderblockUnchanged = !!(spec && spec.light_gray_cinderblock &&
        spec.light_gray_cinderblock.blockW === 48 && spec.light_gray_cinderblock.blockH === 12);
    } else {
      checks.brickPaletteDistinctFromCinder = false;
      checks.brickNotMostlyGrayPixels = false;
      checks.brickCalmHasVerticalJoints = false;
      checks.cinderblockUnchanged = false;
    }

    // Facade layout system exists
    checks.facadeLayoutsExist = typeof crGetFacadeLayoutForComponentFace === 'function' &&
      typeof crComponentFaceLayoutKind === 'function' &&
      typeof CR_FACADE_LAYOUT_CACHE !== 'undefined';
    checks.componentFaceLayoutsExist = checks.facadeLayoutsExist;

    // Sign fit still safe
    if(typeof crFitSignText === 'function'){
      const fit = crFitSignText('COLORADO NATIONAL MONUMENT', 100, 20);
      checks.signBoardFits = fit.overflow === false && fit.lines.length > 0;
      evidence.signFitSample = fit;
    } else { checks.signBoardFits = false; }

    // Draw path: base material first, then decals
    const matWallSrc = typeof crDrawBuildingMaterialWallColumn === 'function' ? String(crDrawBuildingMaterialWallColumn) : '';
    checks.decalsOverlayOnly = matWallSrc.indexOf('crDrawContinuousFacadeTextureColumn') >= 0 &&
      matWallSrc.indexOf('crDrawDecalOverlayColumn') >= 0;

    // Generate city + count facade layouts and decals
    const saved = { seed: game.seed, district: game.district };
    let greyFlipCount = 0;
    let sampledMaterialKeys = {};
    try {
      game.seed = 290144176;
      game.district = 2;
      genCity(game.seed, game.district, game.modifier || '');
      ...
      let storefrontAssemblies = 0, residentialAssemblies = 0, serviceAssemblies = 0;
      let visibleLargeDecals = 0;
      const seenLayouts = {};
      const seenComponents = {};

      for(let y = 1; y < game.MAP_H - 1; y++){
        for(let x = 1; x < game.MAP_W - 1; x++){
          const wt = game.map[y] && game.map[y][x];
          if(!crIsBuildingMaterialWallType(wt)) continue;

          // Gray flip probe: check material resolution stability
          const mk = crBuildingMaterialForCell(x, y);
          sampledMaterialKeys[mk] = (sampledMaterialKeys[mk] || 0) + 1;
          // If a cell has component ownership but resolves to stucco, that's a gray flip
          const comp = crBuildingMaterialComponentForCell(x, y);
          if(comp && comp.materialKey && comp.materialKey !== 'stucco'){
            if(mk === 'stucco') greyFlipCount++;
          }

          // Count facade layouts per component face
          if(comp){
            const cid = comp.componentId || comp.cid || '';
            for(const fd of ['south', 'north', 'east', 'west']){
              const fkey = cid + ':' + fd;
              if(seenLayouts[fkey]) continue;
              const layout = crGetFacadeLayoutForComponentFace(comp, fd);
              if(layout){
                seenLayouts[fkey] = layout.kind;
                if(layout.kind === 'commercial_front' && (fd === 'south' || fd === 'north')){
                  storefrontAssemblies++;
                  seenComponents[cid] = true;
                }
                if(layout.kind === 'residential_front' && (fd === 'south' || fd === 'north')) residentialAssemblies++;
                if(layout.kind === 'service_front') serviceAssemblies++;
                // Count visible decals on visible faces
                if(fd === 'south' || fd === 'north'){
                  for(const d of layout.decals){
                    if(d.key === 'decal_sign_board' || d.key === 'decal_storefront_window' ||
                       d.key === 'decal_glass_door' || d.key === 'decal_front_door' ||
                       d.key === 'decal_house_window' || d.key === 'decal_service_door'){
                      visibleLargeDecals++;
                    }
                  }
                }
          ...
          // Registry-layer assembly counting: D2 storefronts and D3 services are registry
          // buildings using crGetModuleDecalsForFace, not component layouts. Dedupe by bid.
          const cell = game.buildingGrid && game.buildingGrid[y] && game.buildingGrid[y][x];
          if(cell && cell.bid){
            const bid = cell.bid;
            const seenKey = 'reg:' + bid;
            if(!seenLayouts[seenKey]){
              seenLayouts[seenKey] = true; // mark bid as seen BEFORE face loop
              const reg = game.buildingRegistry && game.buildingRegistry[bid];
              if(reg){
                for(const fd of ['south', 'north']){
                  const decals = crGetModuleDecalsForFace(x, y, fd, cell, reg);
                  let hasSign = false, hasStorefront = false, hasService = false;
                  for(const d of decals){
                    if(d.key === 'decal_sign_board') hasSign = true;
                    if(d.key === 'decal_storefront_window' || d.key === 'decal_glass_door') hasStorefront = true;
                    if(d.key === 'decal_service_door') hasService = true;
                    if(fd === 'south' || fd === 'north'){
                      if(d.key === 'decal_sign_board' || d.key === 'decal_storefront_window' ||
                         d.key === 'decal_glass_door' || d.key === 'decal_front_door' ||
                         d.key === 'decal_house_window' || d.key === 'decal_service_door'){
                  ...
                  if(hasStorefront){ storefrontAssemblies++; }
                  else if(hasService){ serviceAssemblies++; }
                  else if(decals.length > 0){ residentialAssemblies++; }
                }
              }
            }
          }
      ...
      metrics.d2LongWall.sampledMaterialKeys = sampledMaterialKeys;
      metrics.d2LongWall.greyFlipCount = greyFlipCount;
      metrics.decalCoverage.storefrontAssemblies = storefrontAssemblies;
      metrics.decalCoverage.residentialAssemblies = residentialAssemblies;
      metrics.decalCoverage.serviceAssemblies = serviceAssemblies;
      metrics.decalCoverage.componentFaceLayouts = Object.keys(seenLayouts).length;
      metrics.decalCoverage.visibleLargeDecals = visibleLargeDecals;

      // Acceptance thresholds: must have visible facade assemblies, not just counts
      checks.playerFacingDecalAreaEnough = visibleLargeDecals >= 8;
      checks.storefrontAssemblyVisible = storefrontAssemblies >= 1;
      checks.residentialAssemblyVisible = residentialAssemblies >= 1;
      checks.serviceAssemblyVisible = serviceAssemblies >= 1;
      checks.materialIdentityStable = greyFlipCount === 0 && checks.materialIdentityStable;
    } finally {
      game.seed = saved.seed;
      game.district = saved.district;
    }
  ...
  return {
    ...
    errors,
    screenshots: [
      'proof-decalintegration3-d2-long-wall.png',
      'proof-decalintegration3-d2-storefront-facade.png',
      'proof-decalintegration3-d2-brick-stability-a.png',
      'proof-decalintegration3-d2-brick-stability-b.png',
      'proof-decalintegration3-d1-residential-facade.png',
      'proof-decalintegration3-d3-service-facade.png',
    ],
    timestamp: new Date().toISOString(),
  };
}

function runFacadeSkins1SelfCheck(){
  if(_crHarnessDepth > 0) return crRunFacadeSkins1SelfCheck();
  return crWithTemporaryState('facadeSkins1', () => crRunFacadeSkins1SelfCheck());
}

function runDecalIntegration4MaterialIdentitySelfCheck(){
  if(_crHarnessDepth > 0) return runDecalIntegration4MaterialIdentitySelfCheckBody();
  return crWithTemporaryState('decalIntegration4MaterialIdentity', () => runDecalIntegration4MaterialIdentitySelfCheckBody());
}

function runDecalIntegration4MaterialIdentitySelfCheckBody(){
  const errors = [];
  const checks = {};
  const evidence = {};
  const metrics = {};
  try {
    ...
    const spec = typeof crMaterialTextureDebugSpec === 'function' ? crMaterialTextureDebugSpec() : null;
    evidence.materialSpecs = spec;
    if(spec && spec.red_brick && spec.light_gray_cinderblock){
      const rb = spec.red_brick;
      const cb = spec.light_gray_cinderblock;
      checks.brickNotCinderGeometry = rb.brickW !== cb.blockW && rb.brickH !== cb.blockH &&
        Math.abs(rb.aspect - cb.aspect) >= 0.6;
      checks.cinderblockUnchanged = cb.blockW === 48 && cb.blockH === 12;
      metrics.geometry = {
        redBrick: { brickW: rb.brickW, brickH: rb.brickH, aspect: rb.aspect },
        cinder: { blockW: cb.blockW, blockH: cb.blockH, aspect: cb.aspect },
      };
    } else {
      checks.brickNotCinderGeometry = false;
      checks.cinderblockUnchanged = false;
    ...
    if(atlas && typeof crSampleMaterialTexturePaletteStats === 'function'){
      ...
      const cinderPal = crSampleMaterialTexturePaletteStats(atlas.material_light_gray_cinderblock);
      evidence.paletteSamples = { brick: brickPal, cinder: cinderPal };
      metrics.palette = { brick: brickPal, cinder: cinderPal };
      if(brickPal && cinderPal){
        checks.brickPaletteDistinctFromCinder = brickPal.warmBrickFraction >= 0.10 && cinderPal.grayBlockFraction >= 0.06 &&
          (brickPal.avgR - brickPal.avgB) >= 45 && (brickPal.avgR - brickPal.avgG) >= 35 &&
          Math.abs(cinderPal.avgR - cinderPal.avgG) < 28;
        metrics.visualDistance = Math.abs(brickPal.avgR - cinderPal.avgR) + Math.abs(brickPal.avgG - cinderPal.avgG);
        checks.brickCinderVisualDistanceEnough = metrics.visualDistance >= 35;
        checks.brickNotMostlyGrayPixels = brickPal.grayBlockFraction < 0.55;
      } else {
        checks.brickPaletteDistinctFromCinder = false;
        checks.brickCinderVisualDistanceEnough = false;
        checks.brickNotMostlyGrayPixels = false;
      }
      checks.brickCalmHasVerticalJoints = typeof crCalmBrickHasVerticalJoints === 'function' &&
        crCalmBrickHasVerticalJoints(atlas.material_red_brick_calm);
    ...
    const di3 = runDecalIntegration3VisualSelfCheck();
    evidence.decalIntegration3Visual = { pass: di3.pass, checks: di3.checks, metrics: di3.metrics };
    const greyFlip = di3.metrics && di3.metrics.d2LongWall ? di3.metrics.d2LongWall.greyFlipCount : null;
    checks.noGreyFlipStillPasses = di3.pass === true && greyFlip === 0;
    checks.facadeLayoutsStillPass = !!(di3.checks && di3.checks.storefrontAssemblyVisible &&
      di3.checks.residentialAssemblyVisible && di3.checks.serviceAssemblyVisible);
    checks.walltextures3OwnershipStillPasses = !!(di3.checks && di3.checks.walltextures3OwnershipStillPasses);
    checks.decalOverlayOnlyStillPasses = !!(di3.checks && di3.checks.decalsOverlayOnly);
    metrics.nestedDi3GreyFlipCount = greyFlip;
  } catch(e) {
    errors.push(String(e && e.message ? e.message : e));
  }
  for(const k of Object.keys(checks)) if(!checks[k]) errors.push('check failed: ' + k);
  ...
  return {
    ...
    errors,
    screenshots: [
      'proof-decalintegration4-brick-identity.png',
      'proof-decalintegration4-cinderblock-identity.png',
      'proof-decalintegration4-brick-vs-cinder-side-by-side.png',
    ],
    timestamp: new Date().toISOString(),
  };
}
...
/* Browser-only self-check lifecycle. Normal callers retain the synchronous
   CR.runFullSelfCheck() contract above; only the ?selfcheck URL uses this path. */
const CR_SELFCHECK_GROUPS = [
  { name: 'core-layout-input', checks: [
    ['layout', runLayoutSelfCheck], ['viewportSafeArea', runViewportSafeAreaSelfCheck], ['portraitUsability', runPortraitUsabilitySelfCheck], ['settingsSafety', runSettingsSafetySelfCheck], ['decorativeProps', runDecorativePropsSelfCheck], ['optionsCleanup', runOptionsCleanupSelfCheck], ['mobileControlReliability', runMobileControlReliabilitySelfCheck], ['movementCollision', runMovementCollisionSelfCheck], ['reachability', runReachabilitySelfCheck],
  ] },
  { name: 'world-facade', checks: [
    ['streetBlockLevel', runStreetBlockLevelSelfCheck], ['d1ParkLandmark', runD1ParkLandmarkSelfCheck], ['buildingModuleFacade', runBuildingModuleFacadeSelfCheck], ['facadePackBridge', runFacadePackBridgeSelfCheck], ['facadePackV2Safe', runFacadePackV2SafeModuleSelfCheck], ['fpvGroundPlaneAlignment', runFpvGroundPlaneAlignmentSelfCheck], ['d2D3FacadeReadabilityFinal', runD2D3FacadeReadabilityFinalSelfCheck], ['buildingSmoothStyle', runBuildingSmoothStyleSelfCheck], ['continuousFacadeTexture', runContinuousFacadeTextureSelfCheck], ['spriteGroundAnchor', runSpriteGroundAnchorSelfCheck], ['facadeArtVocabulary', runFacadeArtVocabularySelfCheck], ['facadeCompositionReadability', runFacadeCompositionReadabilitySelfCheck], ['fpvFacadeTargetPolish', runFpvFacadeTargetPolishSelfCheck], ['fpvWallLineArtifactFix', runFpvWallLineArtifactFixSelfCheck], ['fpvStreetShimmerFix', runFpvStreetShimmerFixSelfCheck], ['streetReadabilityMinimap', runStreetReadabilityMinimapSelfCheck],
  ] },
  { name: 'progression-feedback', checks: [
    ['buildingScalePolish', runBuildingScalePolishSelfCheck], ['earlyDistrictProgression', runEarlyDistrictProgressionSelfCheck], ['levelSelector', runLevelSelectorSelfCheck], ['proceduralLevelValidation', runProceduralLevelValidationSelfCheck], ['fullRunProgression', runFullRunProgressionSelfCheck], ['onboarding', runOnboardingSelfCheck], ['soundFeedback', runSoundFeedbackSelfCheck],
  ] },
  { name: 'controls-render', checks: [
    ['declarativeControls', runDeclarativeControlsSelfCheck], ['visualReadability', runVisualReadabilitySelfCheck], ['visualRectangleRegression', runVisualRectangleRegressionSelfCheck], ['input', runInputSelfCheck], ['levels', runLevelSelfCheck], ['renderFailure', runRenderFailureSelfCheck],
  ] },
];

function crSelfCheckLifecycleSnapshot(lifecycle){
  return {
    version: lifecycle.version, state: lifecycle.state, startedAt: lifecycle.startedAt, completedAt: lifecycle.completedAt,
    currentSection: lifecycle.currentSection, lastCompletedSection: lifecycle.lastCompletedSection,
    completedSections: lifecycle.completedSections.slice(), failedSection: lifecycle.failedSection,
    lastProgressAt: lifecycle.lastProgressAt, result: lifecycle.result, error: lifecycle.error ? { ...lifecycle.error } : null,
  };
}

function crSelfCheckYield(){
...
async function crWithTemporaryStateAsync(label, fn){
  _crHarnessDepth++;
  _crBlockHarnessSave = true;
  const snap = crSnapshotHarnessState();
  try {
    ...
    try {
      crRestoreHarnessState(snap);
      crClearHarnessLeakFromStorage();
      if(snap.state === STATE.TITLE || _selfCheckUrl) crForceSafeTitleAfterHarness();
      if(crIsBenchmarkPlayState()) crForceSafeTitleAfterHarness();
      _selfCheckForcePortrait = false;
      applyMobileControlSettings();
      syncPortraitMenuLabel();
      ...
      crForceSafeTitleAfterHarness();
      console.error('crWithTemporaryStateAsync restore failed', label, re);
    }
    _crHarnessDepth--;
    if(_crHarnessDepth <= 0){
      _crHarnessDepth = 0;
...
function crBuildAsyncFullSelfCheckResult(sections, err0){
  const runtimeClean = window.__crRuntimeErrors.length === err0;
  const pass = Object.values(sections).every(section => section && section.pass) && runtimeClean;
  return { ...sections, pass, build: BUILD_ID, render: sections.renderFailure, runtimeErrors: window.__crRuntimeErrors.slice(), runtimeClean, timestamp: new Date().toISOString() };
}

function crFindFailedSelfCheckSection(sections){
  for(const group of CR_SELFCHECK_GROUPS){
    for(const [key] of group.checks){ if(!sections[key] || !sections[key].pass) return key; }
  }
  return 'runtime-clean';
}

function crSelfCheckTerminalState(state){ return state === 'passed' || state === 'failed' || state === 'timed_out'; }

function crResolveBrowserSelfCheckDone(){
  const lifecycle = window.__crSelfCheckLifecycle;
  if(!lifecycle || !crSelfCheckTerminalState(lifecycle.state) || window.__crSelfCheckDoneResolved) return;
  window.__crSelfCheckDoneResolved = true;
  const payload = {
    state: lifecycle.state, result: lifecycle.result, failedSection: lifecycle.failedSection,
    error: lifecycle.error, completedSections: lifecycle.completedSections.slice(), lifecycle: crSelfCheckLifecycleSnapshot(lifecycle),
  };
  window.__crResolveSelfCheckDone?.(payload);
}

async function runBrowserSelfCheckLifecycle(params){
  const lifecycle = window.__crSelfCheckLifecycle;
  lifecycle.state = 'running';
  lifecycle.startedAt = Date.now();
  lifecycle.lastProgressAt = lifecycle.startedAt;
  try {
    // TEST-ONLY deterministic observer path. It is inert without this explicit URL flag.
    if(params.get('selfcheckhangmeta') === '1'){
      lifecycle.currentSection = 'hang-meta-section';
      lifecycle.lastProgressAt = Date.now();
      return await new Promise(() => {});
    }
    const sections = await crWithTemporaryStateAsync('full-browser-lifecycle', async () => {
      const resultSections = {};
      const err0 = window.__crRuntimeErrors.length;
      for(let groupIndex = 0; groupIndex < CR_SELFCHECK_GROUPS.length; groupIndex++){
        const group = CR_SELFCHECK_GROUPS[groupIndex];
        lifecycle.currentSection = group.name;
        lifecycle.lastProgressAt = Date.now();
        // TEST-ONLY deterministic failure after one real completed group.
        if(params.get('selfcheckfailmeta') === '1' && groupIndex === 1){
          lifecycle.currentSection = 'failure-meta-section';
          lifecycle.lastProgressAt = Date.now();
          throw new Error('test-only self-check lifecycle failure');
        }
        for(const [key, check] of group.checks) resultSections[key] = check();
        lifecycle.lastCompletedSection = group.name;
        lifecycle.completedSections.push(group.name);
        lifecycle.lastProgressAt = Date.now();
        if(groupIndex < CR_SELFCHECK_GROUPS.length - 1) await crSelfCheckYield();
      }
      return crBuildAsyncFullSelfCheckResult(resultSections, err0);
    });
    lifecycle.result = sections;
    lifecycle.completedAt = Date.now();
    lifecycle.lastProgressAt = lifecycle.completedAt;
    if(sections.pass){
      lifecycle.state = 'passed';
    } else {
      lifecycle.state = 'failed';
      lifecycle.failedSection = crFindFailedSelfCheckSection(sections);
      const message = 'self-check section failed: '+lifecycle.failedSection;
      lifecycle.error = { message, stack: new Error(message).stack };
    }
  } catch(e){
    lifecycle.state = 'failed';
    lifecycle.failedSection = lifecycle.currentSection || lifecycle.lastCompletedSection || 'unknown';
    lifecycle.error = { message: String(e && e.message ? e.message : e), stack: String(e && e.stack ? e.stack : e) };
    lifecycle.completedAt = Date.now();
    lifecycle.lastProgressAt = lifecycle.completedAt;
  }
  crResolveBrowserSelfCheckDone();
...
if(_selfCheckUrl){
  const _selfCheckParams = new URLSearchParams(location.search);
  window.__crSelfCheckRan = false;
  window.__crSelfCheckDoneResolved = false;
  window.__crSelfCheckLifecycle = {
    version: 1, state: 'idle', startedAt: null, completedAt: null, currentSection: null,
    lastCompletedSection: null, completedSections: [], failedSection: null, lastProgressAt: null,
    result: null, error: null,
  };
  window.__crSelfCheckDone = new Promise(resolve => { window.__crResolveSelfCheckDone = resolve; });
  const _bootSelfCheckOnce = async ()=>{
    if(window.__crSelfCheckRan) return;
    window.__crSelfCheckRan = true;
    try {
      const result = await runBrowserSelfCheckLifecycle(_selfCheckParams);
      if(!window.__crSelfCheckLifecycle || window.__crSelfCheckLifecycle.state === 'running') return;
      window.__crSelfCheckResult = result || window.__crSelfCheckLifecycle.result;
      showCrSelfCheckOverlay(window.__crSelfCheckResult || { pass: false });
    } catch(e){
      const lifecycle = window.__crSelfCheckLifecycle;
      lifecycle.state = 'failed';
      lifecycle.failedSection = lifecycle.currentSection || lifecycle.lastCompletedSection || 'boot';
      lifecycle.error = { message: String(e && e.message ? e.message : e), stack: String(e && e.stack ? e.stack : e) };
      lifecycle.completedAt = Date.now();
      lifecycle.lastProgressAt = lifecycle.completedAt;
      crResolveBrowserSelfCheckDone();
      ...
      const lifecycle = window.__crSelfCheckLifecycle;
      if(lifecycle && lifecycle.state !== 'running'){
        crResolveBrowserSelfCheckDone();
        crForceSafeTitleAfterHarness();
        if(typeof drawMobileMenu === 'function') drawMobileMenu();
        resize();
...
/* Visual QA debug route — ?visualqa=facades&seed=...&district=...&mobile=on&portraitlayout=1
   Starts directly at the chosen district facing a long wall, dismisses onboarding,
   exposes BUILD_ID and material/decal debug info. Does not run the full selfcheck harness. */
if(new URLSearchParams(location.search).get('visualqa') === 'facades'){
  const _visualqaParams = new URLSearchParams(location.search);
  const _vqaSeed = parseInt(_visualqaParams.get('seed'), 10) || 974659691;
  const _vqaDistrict = parseInt(_visualqaParams.get('district'), 10) || 2;
  window.__crVisualQaReady = false;
  const _bootVisualQaOnce = ()=>{
    if(window.__crVisualQaReady) return;
    if(typeof crSetSelectedStartDistrict !== 'function' || typeof startRun !== 'function') return;
    window.__crVisualQaReady = true;
    crSetSelectedStartDistrict(_vqaDistrict);
    startRun(_vqaSeed);
    state = STATE.PLAY;
    paused = false;
    if(typeof dismissOnboardingHelp === 'function') dismissOnboardingHelp();
    // Place player facing a long material wall
    const G = game;
    const map = G.map || [];
    ...
    for(let y = 1; y < G.MAP_H - 1; y++){
      ...
      for(let x = 2; x < G.MAP_W - 2; x++){
        if(map[y] && map[y][x] > 0) score++;
      }
      if(score > bestScore){ bestScore = score; bestRow = y; }
    }
    player.x = 4.5;
    player.y = bestRow + 0.5;
    player.angle = 0;
    player.dir = 0;
