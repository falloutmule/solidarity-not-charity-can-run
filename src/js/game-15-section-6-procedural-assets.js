...
function crMinimapPrefabCellColor(x, y){
  const cell = game.buildingGrid && game.buildingGrid[y] && game.buildingGrid[y][x];
  const reg = cell && game.buildingRegistry ? game.buildingRegistry[cell.bid] : null;
  if(!reg || reg.renderMode !== 'importedWholeFaceAsset') return null;
  const aid = reg.assetId || '';
  if(aid.indexOf('strip_mall') === 0) return cell.ly === 0 ? '#d6a247' : '#9b6b32';
  if(aid.indexOf('corner_shop') === 0) return cell.lx === 0 ? '#cf7b45' : '#8c4a36';
  if(aid.indexOf('small_service') === 0) return (cell.lx + cell.ly) % 2 ? '#6f7d86' : '#b35d3d';
  if(aid.indexOf('small_house') === 0) return cell.ly === 0 ? '#8f6a4d' : '#5f3f2d';
  if(aid.indexOf('small_office') === 0) return cell.lx === 1 ? '#8aa5b8' : '#526b7b';
  if(aid.indexOf('clinic_house') === 0) return (cell.lx === 1 || cell.ly === 1) ? '#d85a5a' : '#d8cfb2';
  if(aid.indexOf('car') === 0) return '#4d7ea8';
  if(aid.indexOf('dumpster') === 0) return '#2f6d4f';
  if(aid.indexOf('shed') === 0) return '#7a533c';
  if(aid.indexOf('fence_piece') === 0) return '#6b553d';
  return '#a07b52';
}
...
function crMinimapNavCellColor(x, y, v){
  ...
  if(v !== 0){
    const prefabColor = crMinimapPrefabCellColor(x, y);
    if(prefabColor) return prefabColor;
...
function bakeDecorPropTexture(kind, variant, label){
  const key = kind+":"+variant+":"+(label || '');
  ...
  if(label && kind === 'signboard'){
    const lines = String(label).split(/\n+/).slice(0,3);
    ctx.fillStyle = 'rgba(17,18,24,0.68)';
    ctx.fillRect(3, 5, sz[0] - 6, Math.min(sz[1] - 10, 14 + lines.length * 9));
    ...
    for(let i = 0; i < lines.length; i++){
      ctx.fillText(lines[i], sz[0] / 2, startY + i * 9);
    }
    ctx.textAlign = 'start';
