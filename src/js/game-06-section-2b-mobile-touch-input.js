...
var BUILD_ID = 'world1thestand1-givetouch1'; window.BUILD_ID = BUILD_ID;
...
function bindMobileControls(){
  ...
  // ---- GIVE button ----
  // Pointer Events are normally authoritative, but Samsung's compatibility
  // Touch stream can arrive without a usable pointer delivery for a discrete
  // button. Keep a local fallback here (never on document) so that stream is
  // consumed before it can synthesize a canvas/menu tap. The pointer id guard
  // makes the normal Chrome pointer+touch pair one semantic GIVE action.
  let givePointerId = null;
  let giveTouchId = null;
  const clearGivePress = ()=>{
    inp.give = false;
    mg.classList.remove('pr');
  };
  const givePtrDown = e=>{
    if(_controlEditActive) return;
    e.preventDefault(); e.stopPropagation();
    givePointerId = e.pointerId;
    try{ mg.setPointerCapture(e.pointerId); }catch(_e){}
    inp.give = true;
    mg.classList.add('pr');
  };
  const givePtrUp = e=>{
    if(_controlEditActive || (givePointerId !== null && e.pointerId !== givePointerId)) return;
    e.preventDefault(); e.stopPropagation();
    try{ mg.releasePointerCapture(e.pointerId); }catch(_e){}
    givePointerId = null;
    clearGivePress();
  };
  ...
  mg.addEventListener('touchstart', e=>{
    if(_controlEditActive || givePointerId !== null || giveTouchId !== null) return;
    const t = e.changedTouches && e.changedTouches[0];
    if(!t) return;
    e.preventDefault(); e.stopPropagation();
    giveTouchId = t.identifier;
    inp.give = true;
    mg.classList.add('pr');
  }, {passive:false});
  const giveTouchEnd = e=>{
    if(giveTouchId === null) return;
    const ended = Array.from(e.changedTouches || []).some(t=>t.identifier === giveTouchId);
    if(!ended) return;
    e.preventDefault(); e.stopPropagation();
    giveTouchId = null;
    clearGivePress();
  };
  mg.addEventListener('touchend', giveTouchEnd, {passive:false});
  mg.addEventListener('touchcancel', giveTouchEnd, {passive:false});
  mg.addEventListener('click', e=>{ e.preventDefault(); e.stopPropagation(); });
