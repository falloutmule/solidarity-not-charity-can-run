...
function createFixture({ west = false } = {}) {
  ...
  const faces = Object.fromEntries(Object.keys(files).map((name) => [name, `source/${name}.png`]));
  fs.writeFileSync(path.join(directory, 'building.json'), `${JSON.stringify({ schema: 'snc-building-source-v1', id: 'fixture_pantry_001', footprint: { widthCells: 3, depthCells: 2 }, faces }, null, 2)}\n`, 'utf8');
  return directory;
}

module.exports = { ROOT, createFixture };
