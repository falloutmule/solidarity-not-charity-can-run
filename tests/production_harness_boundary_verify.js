...
const harness = combine(manifest, { includeHarness: true });
...
for (const symbol of ['globalThis.CR = window.CR', 'runFullSelfCheck', '__crVisualQaReady', 'selfcheckhangmeta', 'id="crselfcheck"', '#crselfcheck']) {
  ...
  assert(harness.includes(symbol), `test artifact omitted harness symbol: ${symbol}`);
}
...
assert(!harness.includes('SNC_TEST_HARNESS_BEGIN'), 'test artifact must receive executable harness source, not marker text');

console.log(JSON.stringify({ pass: true, productionBytes: Buffer.byteLength(production), harnessBytes: Buffer.byteLength(harness) }));
