...
**Gameplay baseline:** `facadetexture1` — see **`../PROJECT_STATUS.md`**. **Source:** edit `src/`, run `npm run build`.
...
| 42 | Building visual reset / smooth wall style | [guards/BUILDING_VISUAL_RESET_SMOOTH_WALL_STYLE_REPORT.md](guards/BUILDING_VISUAL_RESET_SMOOTH_WALL_STYLE_REPORT.md) | `buildingsmooth1` | `CR.runBuildingSmoothStyleSelfCheck` + `proof-buildingsmooth-*.png/json` | Yes (building wall art only) |
| 43 | Continuous facade texture / no panel gaps | [guards/CONTINUOUS_FACADE_TEXTURE_NO_PANEL_GAPS_REPORT.md](guards/CONTINUOUS_FACADE_TEXTURE_NO_PANEL_GAPS_REPORT.md) | `facadetexture1` | `CR.runContinuousFacadeTextureSelfCheck` + `proof-facadetexture-*.png/json` | Yes (building wall art only) |
