...
function exportToTiled(level) {
  const collision = level.mapRows.flatMap((row) => Array.from(row, (cell) => cell === '0' ? 0 : cell === '1' ? 1 : cell === '2' ? 3 : 2));
  let id = 1;
  ...
  const environmentObjects = (level.environmentObjects || []).map((object) => {
    const entry = makeObject(id++, object.id, 'solid-height', object.x * TILE_SIZE, object.y * TILE_SIZE, { assetId: object.assetId, rotationQ: object.rotation, widthCells: object.widthCells, depthCells: object.depthCells }, object.widthCells * TILE_SIZE, object.depthCells * TILE_SIZE);
    entry.rotation = object.rotation * 90;
    return entry;
  });
  ...
  return {
    compressionlevel: -1, infinite: false, layers: [
      { id: 1, name: 'Collision', type: 'tilelayer', visible: true, opacity: 1, x: 0, y: 0, width: level.grid.width, height: level.grid.height, data: collision, properties: properties({ sncEncoding: '0=walkable,1=building,2=concrete,3=low-block' }) },
      objectLayer('Buildings', buildingObjects), objectLayer('Environment', environmentObjects), objectLayer('PlayerStart', [player]), objectLayer('Pickups', pickupObjects), objectLayer('NPCs', npcObjects), objectLayer('Props', propObjects), objectLayer('Exit', [exit]), objectLayer('Metadata', [metadata])
    ],
    nextlayerid: 10, nextobjectid: id, orientation: 'orthogonal', renderorder: 'right-down', tiledversion: '1.11', tileheight: TILE_SIZE, tilewidth: TILE_SIZE, type: 'map', version: '1.10', width: level.grid.width, height: level.grid.height
  };
}
...
function fromTiled(map, baseline) {
  ...
  const rows = [];
  for (let y = 0; y < map.height; y += 1) rows.push(collision.data.slice(y * map.width, (y + 1) * map.width).map((value) => value === 0 ? '0' : value === 1 ? '1' : value === 2 ? '8' : value === 3 ? '2' : (() => { throw new Error(`unknown Collision tile value ${value}`); })()).join(''));
  ...
  const environmentLayer = readObjects(layers.Environment, 'Environment');
  assert(environmentLayer.length === (baseline.environmentObjects || []).length, 'environment object count must match baseline for District 1 companion import');
  const environmentObjects = environmentLayer.map((object) => {
    assert(object.type === 'solid-height', 'Environment layer may contain only solid-height objects');
    assert(object.x % TILE_SIZE === 0 && object.y % TILE_SIZE === 0, 'environment object must snap to the 32px grid');
    const rotation = Number(required(object, 'rotationQ')), widthCells = Number(required(object, 'widthCells')), depthCells = Number(required(object, 'depthCells'));
    assert(Number.isInteger(rotation) && rotation >= 0 && rotation <= 3 && widthCells === 1 && depthCells === 1, 'environment dimensions or rotation invalid');
    assert(object.width === TILE_SIZE && object.height === TILE_SIZE, 'environment footprint dimensions must match properties');
    const assetId = required(object, 'assetId'); assert(currentAssetIds().has(assetId), `unknown environment asset ID: ${assetId}`);
    return { id: object.name, assetId, x: object.x / TILE_SIZE, y: object.y / TILE_SIZE, rotation, widthCells, depthCells };
  });
  ...
  imported.mapRows = rows; imported.buildings = buildings; imported.environmentObjects = environmentObjects;
  const importedPickupRows = pointItems('Pickups', 'pickup', (object) => ({ id: object.name, x: object.x / TILE_SIZE, y: object.y / TILE_SIZE, amt: Number(required(object, 'amt')), socketId: valueOf(object, 'socketId') }));
  if(Array.isArray(baseline.canSockets)){
  ...
  for (const object of environmentObjects) { const key = `${object.x},${object.y}`; assert(rows[object.y] && rows[object.y][object.x] === '2', 'environment footprint must own low-block Collision cell'); assert(!occupied.has(key), 'environment footprints must not overlap'); occupied.add(key); }
  for (const entity of [imported.playerStart, imported.exit, ...(imported.canSockets || imported.pickups), ...imported.npcs, ...imported.props]) { const x = Math.floor(entity.x), y = Math.floor(entity.y); assert(rows[y] && rows[y][x] === '0', 'spawn, exit, and entities must be in walkable Collision cells'); }
  return imported;
}
