...
function previewHtml(compiled) {
  ...
  return `<!doctype html><meta charset="utf-8"><title>${escapeHtml(asset.id)} building preview</title><style>body{margin:24px;background:#17130f;color:#eadfc7;font:16px/1.45 system-ui,sans-serif}h1,h2,h3{color:#ffdc71}code{font-size:11px;overflow-wrap:anywhere}figure{display:inline-block;vertical-align:top;margin:8px;max-width:360px}img,canvas{image-rendering:pixelated;image-rendering:crisp-edges;border:1px solid #8d7643;background:#24211b;max-width:100%;height:auto}figcaption{max-width:320px}table{border-collapse:collapse}th,td{border:1px solid #65583d;padding:6px;text-align:left}.grid{display:flex;flex-wrap:wrap;gap:16px}</style><h1>${escapeHtml(asset.id)}</h1><p>Footprint: <strong>${asset.footprint.wCells}×${asset.footprint.hCells} cells</strong> · atlas ${atlas.width}×${atlas.height} · <code>${asset.atlas.sha256}</code></p><h2>Source faces</h2><div class="grid">${sourceFaces}</div><h2>Packed atlas</h2><img src="${asset.atlas.dataUri}" alt="packed atlas"><h2>Side mapping</h2><table><tr><th>world side</th><th>asset</th><th>reuse</th><th>mirror</th></tr>${faceRows}</table><h2>Quarter-turn preview</h2><div class="grid">${rotations}</div><script>const atlas=new Image();atlas.src=${JSON.stringify(asset.atlas.dataUri)};const slices=${JSON.stringify(asset.faceAssets)};const faces=${JSON.stringify(asset.faces)};const order=['south','east','north','west'];function draw(c,r){const x=c.getContext('2d');x.imageSmoothingEnabled=false;x.fillStyle='#24211b';x.fillRect(0,0,c.width,c.height);const side=order[(r)%4];const face=faces[side],s=slices[face.assetRef].slice;const scale=Math.min((c.width-30)/s.w,(c.height-50)/s.h);const w=Math.round(s.w*scale),h=Math.round(s.h*scale);x.drawImage(atlas,s.x,s.y,s.w,s.h,Math.round((c.width-w)/2),22,w,h);x.fillStyle='#ffdc71';x.font='14px monospace';x.fillText(side+(face.reuse?' ← '+face.reuse:''),12,c.height-12)}atlas.onload=()=>document.querySelectorAll('canvas.rotation').forEach(c=>draw(c,Number(c.dataset.rotation)))</script>`;
}
