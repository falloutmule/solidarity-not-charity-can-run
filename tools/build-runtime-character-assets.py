...
MAX_HEIGHT = 192
PAD = 3
CHARACTER_WORLD_HEIGHT_CLASSES = {
    'standingComposite': 0.78,
    'seatedSlumped': 0.68,
}
...
def load_manifest():
    ...
    class_counts = {class_id: 0 for class_id in CHARACTER_WORLD_HEIGHT_CLASSES}
    for asset in manifest['assets']:
        display_height = asset.get('displayHeightScale')
        world_height_class = asset.get('worldHeightClass')
        ...
        if world_height_class not in CHARACTER_WORLD_HEIGHT_CLASSES:
            raise ValueError(f"{asset.get('assetId', 'unknown')}: unknown worldHeightClass")
        class_counts[world_height_class] += 1
        if 'worldHeight' in asset:
            raise ValueError(f"{asset.get('assetId', 'unknown')}: source manifests use worldHeightClass; per-asset worldHeight is forbidden")
        ...
        if world_height_class == 'seatedSlumped' and contact_row is None:
            raise ValueError(f"{asset.get('assetId', 'unknown')}: seatedSlumped requires groundContactSourceY")
    if class_counts != {'standingComposite': 15, 'seatedSlumped': 1}:
        raise ValueError('approved cast requires fifteen standingComposite and one seatedSlumped asset')
    return manifest
...
def build(write_runtime):
    ...
    for asset in manifest['assets']:
        ...
        record = {
            ...
            'worldHeight': CHARACTER_WORLD_HEIGHT_CLASSES[asset['worldHeightClass']],
    ...
    module_text = '\n'.join([
        ...
        '(function(root){',
        "  'use strict';",
        '  const SNC_CHARACTER_WORLD_HEIGHT_CLASSES = Object.freeze(' + json.dumps(CHARACTER_WORLD_HEIGHT_CLASSES, separators=(',', ': ')) + ');',
        ...
        '  }',
        '  root.SNC_CHARACTER_WORLD_HEIGHT_CLASSES = SNC_CHARACTER_WORLD_HEIGHT_CLASSES;',
