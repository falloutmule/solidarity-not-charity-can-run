#!/usr/bin/env python3
"""Build deterministic soft-alpha foliage assets for World 1."""
import argparse
import base64
import hashlib
import io
...
MANIFEST_PATH = ROOT / 'authoring' / 'foliage' / 'game-ready' / 'runtime-foliage-assets.json'
OUTPUT_PATH = ROOT / 'src' / 'imported-handoff-assets' / 'runtime-foliage-assets.js'
...
def crop_runtime(source):
    ...
    # The pack has a uniform export margin. Crop it, but preserve every
    # nonzero alpha value: foliage needs its supplied soft silhouettes.
...
def load_manifest():
    ...
    if manifest.get('schema') != 'snc-runtime-foliage-assets-v1':
        raise ValueError('unexpected foliage manifest schema')
    ...
    if not classes or any(not isinstance(row.get('worldHeight'), (int, float)) or row['worldHeight'] <= 0 for row in classes.values()):
        raise ValueError('foliage display classes require positive worldHeight values')
    assets = manifest.get('assets', [])
    if not assets or len({asset.get('assetId') for asset in assets}) != len(assets):
        raise ValueError('foliage asset IDs must be unique')
    for asset in assets:
...
def build(write_runtime):
    ...
    for asset in sorted(manifest['assets'], key=lambda row: row['assetId']):
        ...
        if runtime_details['alphaHistogram']['partial'] == 0:
            raise ValueError(f"{asset['assetId']}: foliage must retain soft partial alpha")
        ...
        if write_runtime:
            ...
            raise ValueError(f"{asset['assetId']}: runtime derivative drift; run foliage:build-runtime")
        expected = {
            'runtimeSha256': sha256(runtime_bytes), 'runtimeSize': runtime_details['size'],
            'runtimeAlphaBounds': runtime_details['alphaBounds'], 'runtimeAlphaHistogram': runtime_details['alphaHistogram'],
        }
        ...
        records.append({
            ...
            'alphaHistogram': runtime_details['alphaHistogram'], 'sha256': expected['runtimeSha256'],
            'dataUri': 'data:image/png;base64,' + base64.b64encode(runtime_bytes).decode('ascii'),
        })
    if write_runtime and changed_manifest:
        MANIFEST_PATH.write_text(json.dumps(manifest, indent=2) + '\n', encoding='utf-8')
    module_text = '\n'.join([
        '/* GENERATED by tools/build-runtime-foliage-assets.py; source: authoring/foliage/game-ready/runtime-foliage-assets.json. DO NOT EDIT. */',
        ...
        '  root.SNC_RUNTIME_FOLIAGE_ASSET_REGISTRY = Object.freeze(registry);', '})(globalThis);', ''
    ])
    if write_runtime:
        ...
        raise ValueError('generated foliage registry drift; run foliage:build-runtime')
    return records
...
if __name__ == '__main__':
    try:
        ...
        print(f'runtime-foliage-assets: {error}', file=sys.stderr)
        sys.exit(1)
