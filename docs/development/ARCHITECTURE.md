...
`BUILD_ID` identifies behaviorally distinct builds. The current build is `inputfallback1`. Documentation-only, test-only, and repository-only work does not change it. A runtime behavior change requires a new ID and focused proof.
