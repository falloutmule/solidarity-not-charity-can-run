# Documentation index — SNC Can Run

Navigation for maintainers and Hermes handoffs.

| Doc | Purpose |
|-----|---------|
| [ai-safe-constitution.md](ai-safe-constitution.md) | Edit rules for `index.html` |
| [harness-overview.md](harness-overview.md) | `window.CR` self-checks + Playwright |
| [github-pages.md](github-pages.md) | Deploy URL, cache bust, self-check |
| [mobile-layout-and-controls.md](mobile-layout-and-controls.md) | Frozen portrait UI + control guards |
| [q1k3-lessons.md](q1k3-lessons.md) | q1k3 reference; what we borrowed |
| [handoff-rules.md](handoff-rules.md) | Travis / Hermes working contract |

## Also at repo root

- **`PROJECT_STATUS.md`** — baseline, cards, blockers  
- **`SOURCE_RELEASE_POLICY.md`** — shipped artifact policy  
- **`reports/README.md`** — guard card report index  
- **`src/README.md`** — non-runtime module scaffold